...
use query_types::{
    ...
    CoinMetadataArgs, CoinMetadataQuery, DefaultIotaNSNameQuery, DefaultIotaNSNameQueryArgs,
    DryRunArgs, DryRunQuery, DynamicFieldArgs, DynamicFieldConnectionArgs, DynamicFieldQuery,
    DynamicFieldsOwnerQuery, DynamicObjectFieldQuery, EpochSummaryArgs, EpochSummaryQuery,
    ...
    ObjectsQueryArgs, PackageArgs, PackageByNameArgs, PackageByNameQuery, PackageCheckpointFilter,
    PackageQuery, PackageVersionsArgs, PackageVersionsQuery, PackagesQuery, PackagesQueryArgs,
    PageInfo, ProtocolConfigQuery, ProtocolConfigs, ProtocolVersionArgs, ResolveIotaNSQuery,
    ResolveIotaNSQueryArgs, ServiceConfig, ServiceConfigQuery, TransactionBlockArgs,
...
impl Client {
    ...
    /// Get the address for the provided IotaNS domain name.
    pub async fn resolve_iotans_to_address(&self, domain: &str) -> Result<Option<Address>> {
        let operation = ResolveIotaNSQuery::build(ResolveIotaNSQueryArgs { name: domain });
        ...
        Ok(response
            .data
            .and_then(|d| d.resolve_iotans_address)
            .map(|a| a.address))
    }

    /// Get the default IotaNS domain name for the provided address.
    pub async fn default_iotans_name(&self, address: Address) -> Result<Option<String>> {
        let operation = DefaultIotaNSNameQuery::build(DefaultIotaNSNameQueryArgs { address });
        ...
        Ok(response
            ...
            .and_then(|a| a.default_iotans_name))
    }
}
