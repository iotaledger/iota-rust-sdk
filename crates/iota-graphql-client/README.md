...
When using `cynic` and `iota-graphql-client`, you will need to register the schema by calling `iota-graphql-client-build::register_schema` in a `build.rs` file. See [iota-graphql-client-build](https://github.com/iotaledger/sui-rust-sdk/tree/master/crates/iota-graphql-client-build) for more information.
