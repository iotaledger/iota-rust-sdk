...
/// Emit a client-backed transaction builder object for the given client type.
///
/// Uniffi has no generics, so the Rust `TransactionBuilder<C>` becomes one
/// object per client. Everything but the dry-run result type is identical.
macro_rules! client_transaction_builder {
    (
        $(#[$meta:meta])*
        $name:ident: $client:ty,
        dry_run -> $dry_run_result:ty = |$dry_run:ident| $convert:expr
    ) => {
        $(#[$meta])*
        #[derive(derive_more::From, uniffi::Object)]
        pub struct $name(pub RwLock<iota_sdk::transaction_builder::TransactionBuilder<$client>>);

        impl $name {
            fn read<F, T>(&self, f: F) -> T
            where
                F: FnOnce(&iota_sdk::transaction_builder::TransactionBuilder<$client>) -> T,
            {
            ...
            where
                F: FnOnce(&mut iota_sdk::transaction_builder::TransactionBuilder<$client>) -> T,
            {
        ...
        impl $name {
            ...
            /// `pay()`. For a single recipient, consider
            /// using `send_iota()` instead.
            ...
            /// `authorize_upgrade()`. The result assigned
            ...
            /// `authorize_upgrade()`,
            /// `upgrade()` and
            /// `commit_upgrade()` using the digest from the
            ...
            pub async fn finish(&self) -> Result<Transaction> {
                Ok(Transaction(self.read(|builder| builder.clone()).finish().await?))
            }
            ...
            pub async fn dry_run(&self, skip_checks: bool) -> Result<$dry_run_result> {
                let $dry_run = self.read(|builder| builder.clone()).dry_run(skip_checks).await?;
                Ok($convert)
            }
...
#[cfg(feature = "grpc")]
client_transaction_builder! {
    /// A builder for creating transactions which uses a gRPC client to
    /// automatically resolve inputs. Use `finish` to finalize the transaction
    /// data.
