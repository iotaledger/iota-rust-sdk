...
impl Version {
    ...
    pub fn new_congested_with_suggested_gas_price(suggested_gas_price: u64) -> Self {
        Self(iota_sdk::types::Version::new_congested_with_suggested_gas_price(suggested_gas_price))
    }
    ...
    pub fn get_congested_version_suggested_gas_price(&self) -> u64 {
        self.0.get_congested_version_suggested_gas_price()
    }

    /// Returns a new version that is greater than all versions
    /// in `inputs`, assuming this operation will not overflow.
    #[uniffi::constructor]
    pub fn lamport_increment(inputs: Vec<Arc<Self>>) -> Self {
        Self(iota_sdk::types::Version::lamport_increment(
