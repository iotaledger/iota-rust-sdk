...
mod tests {
    ...
    fn empty_filters() -> FilterConfig {
        FilterConfig {
            include_failed_txs: true,
            tx_function: None,
            tx_sender: None,
            event_type: None,
            event_sending_module: None,
            event_package_id: None,
        }
    }

    #[test]
    fn is_event_only_with_event_type() {
        let mut f = empty_filters();
        f.event_type = Some("0x2::mod::Event".to_string());
        assert!(f.is_event_only());
    }

    #[test]
    fn is_event_only_false_when_tx_function_set() {
        let mut f = empty_filters();
        f.event_type = Some("0x2::mod::Event".to_string());
        f.tx_function = Some("0x2::mod::func".to_string());
        assert!(!f.is_event_only());
    }

    #[test]
    fn is_event_only_false_with_no_filters() {
        assert!(!empty_filters().is_event_only());
    }

    #[test]
    fn derived_tx_function_from_event_type() {
        let mut f = empty_filters();
        f.event_type = Some("0x6d2c::registry::NameRecordAddedEvent".to_string());
        assert_eq!(f.derived_tx_function(), Some("0x6d2c".to_string()));
    }

    #[test]
    fn derived_tx_function_prefers_tx_function() {
        let mut f = empty_filters();
        f.tx_function = Some("0xabc::mod::func".to_string());
        f.event_type = Some("0xdef::mod::Event".to_string());
        assert_eq!(
            f.derived_tx_function(),
            Some("0xabc::mod::func".to_string())
        );
    }

    #[test]
    fn derived_tx_function_falls_back_to_package_id() {
        let mut f = empty_filters();
        f.event_package_id = Some("0x123".to_string());
        assert_eq!(f.derived_tx_function(), Some("0x123".to_string()));
    }

    #[test]
    fn derived_tx_function_none_when_no_filters() {
        assert_eq!(empty_filters().derived_tx_function(), None);
    }
}
