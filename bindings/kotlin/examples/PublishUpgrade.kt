...
fun main() = runBlocking {
  try {
    ...
    val sigPublish = UserSignature.newSimple(privateKey.trySignSimple(txPublish.signingDigest()))
    ...
    for (changedObj in effectsPublish.asV1().changedObjects) {
      if (changedObj.outputState is ObjectOut.ObjectWrite) {
        ...
        val obj: Object =
            client.`object`(objectId, null) ?: throw Exception("Missing object ${objectId.toHex()}")
    ...
    val sigUpgrade = UserSignature.newSimple(privateKey.trySignSimple(txUpgrade.signingDigest()))
