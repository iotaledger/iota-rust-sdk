...
fun main() = runBlocking {
    try {
        ...
        val simulated = client.simulateTransaction(txn)
        val executionError = simulated.executionError
        if (executionError != null) {
            println("Simulation aborted: ${executionError.source}")
        } else {
            println(
                "Simulation succeeded: ${simulated.commandResults?.size ?: 0} command result(s), " +
                    "suggested gas price ${simulated.suggestedGasPrice}"
            )
        }
