...
struct GrpcExecuteTransactionExample {
  static func main() async throws {
    ...
    let simulated = try await client.simulateTransaction(
      transaction: txn, skipChecks: false, readMask: nil)
    if let executionError = simulated.executionError {
      print("Simulation aborted: \(executionError.source ?? "")")
    } else {
      print(
        "Simulation succeeded: \(simulated.commandResults?.count ?? 0) command result(s), "
