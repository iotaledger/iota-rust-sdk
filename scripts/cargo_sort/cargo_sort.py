...
def get_package_names_from_cargo_tomls(directory: str, ignored_patterns: list = None) -> dict:
    ...
    def extract_package_name(file_path: str, names_dict: dict, **_kwargs):
...
def find_all_cargo_tomls(directory: str, ignored_patterns: list = None) -> list:
    ...
    # Skips Cargo.toml files that declare their own [workspace] (standalone
    # workspace roots) unless they are the root workspace Cargo.toml.
    cargo_tomls = []
    ignored_patterns = ignored_patterns or []
    ignored_regexes = [re.compile(p) for p in ignored_patterns]
    root_cargo_toml = os.path.join(os.path.normpath(directory), 'Cargo.toml')
...
def process_cargo_toml_sort(file_path: str, internal_crates_dict: dict, debug: bool, target_dir: str = None):
    ...
    def is_internal(node):
        # Check if a dependency is internal to this workspace.
        if node.name not in internal_crates_dict:
            return False
        # If the dependency specifies an explicit path, verify it resolves
        # to within the workspace. This avoids false positives when an external
    ...
    class Section(object):
        ...
        def add_node(self, node):
            ...
            if ('dependencies' not in self.line) or (not is_internal(node)):
    ...
    # Array names that are safe to sort regardless of section
    ...
    class Node(object):
        ...
        def should_sort_array(self):
            # Only sort arrays where order is known to be insignificant:
            # - Feature definition arrays in [features]
            # - 'members' and 'exclude' arrays in [workspace]
            if self.name in SORTABLE_ARRAYS:
                return True
            if self.section_name == '[features]':
                return True
            return False
            ...
            if self.is_multiline and len(self.lines) > 2 and self.should_sort_array():
                ...
                for line in interior:
                    ...
                    if not stripped or stripped.startswith('#'):
                        ...
                        groups.append((pending_comments, line))
                        pending_comments = []
                groups.sort(key=lambda g: g[1])
                sorted_interior = []
                for comments, entry in groups:
                    sorted_interior.extend(comments)
